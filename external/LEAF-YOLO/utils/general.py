...
def bbox_nwd(box1, box2, x1y1x2y2=False, C=12.8, eps=1e-7):
    box2 = box2.T
    ...
    w2_sq = center_distance + wh_distance

    nwd = torch.exp(-torch.sqrt(w2_sq + eps) / C)
    return nwd

def shape_iou(box1, box2, x1y1x2y2=True, scale=0.0, eps=1e-7):
    # Returns the IoU of box1 to box2. box1 is 4, box2 is nx4
    box2 = box2.T
...
def inner_shape_iou(box1, box2, x1y1x2y2=False, scale=0.0, eps=1e-7,ratio=1.15):
    # Returns the IoU of box1 to box2. box1 is 4, box2 is nx4
    box2 = box2.T
    ...
    #Inner-IOU
    inner_b1_x1, inner_b1_x2 = box1[0] - (box1[2] / 2)*ratio, box1[0] +(box1[2] / 2)*ratio
    inner_b1_y1, inner_b1_y2 = box1[1] - (box1[3] / 2)*ratio,  box1[1] + (box1[3] / 2)*ratio
    inner_b2_x1, inner_b2_x2 = box2[0] - (box2[2] / 2)*ratio, box2[0] + (box2[2] / 2)*ratio
    inner_b2_y1, inner_b2_y2 = box2[1] - (box2[3] / 2)*ratio, box2[1] + (box2[3] / 2)*ratio
    ...
    #Shape-IoU    #Shape-IoU    #Shape-IoU    #Shape-IoU    #Shape-IoU    #Shape-IoU    #Shape-IoU    #Shape-IoU    #Shape-IoU
    iou = inner_iou - distance - 0.5 * ( shape_cost)
    return iou  # IoU
