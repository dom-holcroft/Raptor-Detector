...
from utils.general import bbox_iou, bbox_alpha_iou, bbox_nwd, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy, shape_iou, inner_shape_iou
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # iou = shape_iou(pbox.T, tbox[i], x1y1x2y2=False, scale=0.0)  # iou(prediction, target)
                selected_tbox=tbox[i]
                iou = inner_shape_iou(pbox.T, selected_tbox, x1y1x2y2=False, scale=0.0)
                
                # 2. Calculate NWD
                nwd = bbox_nwd(pbox.T, selected_tbox, x1y1x2y2=False, C=12.8)
                
                # 3. Get the ratio from hyperparameters 
                nwd_ratio = self.hyp.get('nwd_ratio', 0.0)
                
                # 4. Blend the metrics
                hybrid_metric = (1.0 - nwd_ratio) * iou + nwd_ratio * nwd

                # Calculate final box loss using the hybrid metric
                lbox += (1.0 - hybrid_metric).mean()

                # Objectness
                tobj[b, a, gj, gi] = (1.0 - self.gr) + self.gr * hybrid_metric.detach().clamp(0).type(tobj.dtype)
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # 1. Calculate Standard/Shape IoU
                iou = inner_shape_iou(pbox.T, selected_tbox, x1y1x2y2=False, scale=0.0)
                
                # 2. Calculate NWD
                nwd = bbox_nwd(pbox.T, selected_tbox, x1y1x2y2=False, C=12.8)
                
                # 3. Get the ratio from hyperparameters (default to 0.0 if not set, meaning pure IoU)
                nwd_ratio = self.hyp.get('nwd_ratio', 0.0)
                ...
                # Objectness (Use hybrid metric to train confidence score)
                tobj[b, a, gj, gi] = (1.0 - self.gr) + self.gr * hybrid_metric.detach().clamp(0).type(tobj.dtype)
